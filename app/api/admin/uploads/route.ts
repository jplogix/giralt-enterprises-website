...
export async function POST(request: Request) {
  ...
  try {
    await github.uploadFileToRepo(REPO, BRANCH, filePath, contentBase64, `Add image ${filePath}`)

    // update metadata file
    const jsonPath = 'data/images.json'
    await github.updateJsonFile(REPO, BRANCH, jsonPath, (current: any) => {
    ...
    return NextResponse.json({ ok: true, path: `/images/giralt/${category}/${filename}` })
  } catch (err: any) {
