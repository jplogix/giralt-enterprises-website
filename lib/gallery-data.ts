...
): Promise<{ image: GalleryImage; commitSuccess: boolean }> {
	const data = getGalleryData();
...
export async function updateImage(
	id: string,
	updates: Partial<Omit<GalleryImage, "id" | "createdAt">>,
): Promise<{ image: GalleryImage | null; commitSuccess: boolean }> {
	const data = getGalleryData();
...
export async function deleteImage(
	id: string,
): Promise<{ success: boolean; commitSuccess: boolean }> {
	const data = getGalleryData();
	const index = data.images.findIndex((img) => img.id === id);
...
export async function updateCategory(
	id: string,
	updates: Partial<Omit<GalleryCategory, "id">>,
